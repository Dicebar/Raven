...
local function UpdateBar(bp, vbp, bg, b, icon, timeLeft, duration, count, btype, ttType, ttID, ttUnit, ttCaster, isMine)
	...
	if bar then
		...
		local inStartBar = sfxBar
			and not b.startReady
			and ((elapsed >= (b.delayTime or 0)) and ((b.startEffectTime == 0) or (elapsed <= ((b.delayTime or 0) + (b.startEffectTime or 5)))))
		...
		if inStartBG and not vbp.selectAll then -- check filters for auto bar group start effects
			...
			inStartBG = (b.isPlayer and vbp.selectPlayer)
				or (b.isPet and vbp.selectPet)
				or (b.isBoss and vbp.selectBoss)
				or (b.isDispel and vbp.selectDispel)
				or (b.isStealable and vbp.selectStealable)
				or (b.isPoison and vbp.selectPoison)
				or (b.isCurse and vbp.selectCurse)
				or (b.isMagic and vbp.selectMagic)
				or (b.isDisease and vbp.selectDisease)
				or (b.isEnrage and vbp.selectEnrage)
		end

		if elapsed == 0 then
			bar.shineStart = false
			bar.sparkleStart = false
			bar.pulseStart = false -- reset flags for when bars are started or restarted
			bar.shineExpiring = false
			bar.sparkleExpiring = false
			bar.pulseExpiring = false -- and for when bars are expiring
			bar.shineEnd = false
			bar.sparkleEnd = false
			bar.pulseEnd = false -- and for when bars are nearly finished
			bar.shineReady = false
			bar.sparkleReady = false
			bar.pulseReady = false -- and for ready bars
		...
		if sparkleStart then
			bar.sparkleStart = true
		end
		if sparkleExpiring then
			bar.sparkleExpiring = true
		end
		if sparkleEnd then
			bar.sparkleEnd = true
		end
		if sparkleReady then
			bar.sparkleReady = true
		end
		...
		if pulseExpiring then
			bar.pulseExpiring = true
		end
		if pulseEnd then
			bar.pulseEnd = true
		end
		...
		if (inFinishBG and vbp.splash) or (inFinishBar and b.splash) then
			bat.splash = true
		end --

		local isFlashing = (IsOn(b.flashBar) and (b.flashBar == MOD:CheckCondition(b.flashCondition))) -- conditional flashing
			or (vbp.flashStart and inStartBG)
			or (b.flashStart and inStartBar) -- bar group or custom bar start effect
			or (vbp.flashExpiring and inExpireBG)
			or (b.flashExpiring and inExpireBar) -- bar group or custom bar expire effect
			or (b.flashReady and b.startReady) -- ready bar flash option
		MOD.Nest_SetFlash(bar, isFlashing)

		local isGlowing = (IsOn(b.glowBar) and (b.glowBar == MOD:CheckCondition(b.glowCondition))) -- conditional glow
			or (vbp.glowStart and inStartBG)
			or (b.glowStart and inStartBar) -- bar group or custom bar start effect
			or (vbp.glowExpiring and inExpireBG)
			or (b.glowExpiring and inExpireBar) -- bar group or custom bar expire effect
			or (b.glowReady and inReadyBar) -- custom bar ready effect
		MOD.Nest_SetGlow(bar, isGlowing)

		local isDesaturated = (vbp.desatStart and inStartBG)
			or (b.desatStart and inStartBar) -- bar group or custom bar start effect
			or (vbp.desatExpiring and inExpireBG)
			or (b.desatExpiring and inExpireBar) -- bar group or custom bar expire effect
			or (b.desaturateReadyIcon and inReadyBar) -- custom bar ready effect
		...
		if not MOD.db.profile.muteSFX then -- make sure sounds are not muted
			...
			if
				not bar.soundExpire
				and ((inExpireBG and (vbp.soundSpellExpire or (vbp.soundAltExpire ~= "None"))) or (inExpireBar and (b.soundSpellExpire or (b.soundAltExpire ~= "None"))))
			then
			...
			if not bar.soundEnd and ((inFinishBG and (vbp.soundSpellEnd or (vbp.soundAltEnd ~= "None"))) or (inFinishBar and (b.soundSpellEnd or (b.soundAltEnd ~= "None")))) then
...
function MOD:CheckCastBy(caster, cb)
	...
	return ((cb == "player") and isMine)
		or (cb == "anyone")
		or ((cb == "pet") and isPet)
		or ((cb == "other") and not isOurs)
		or ((cb == "ours") and isOurs)
		or ((cb == "nother") and not (isOurs or isTarget))
		or ((cb == "target") and isTarget)
		or ((cb == "focus") and isFocus)
end
...
local function DetectNewBuffs(unit, n, aura, isBuff, bp, vbp, bg)
  ...
  local isStackable = aura[3] > 0
    ...
    or (bp.detectStackable and isStackable)
		...
		or (bp.detectEnrageBuffs and isEnrage)
		or (bp.detectMagicBuffs and isMagic)
		or (bp.detectEffectBuffs and isEffect)
		or (bp.detectAlertBuffs and isAlert)
		or (bp.detectWeaponBuffs and isWeapon)
		or (bp.detectTracking and isTracking)
		or (bp.detectResources and isResource)
		or (bp.detectMountBuffs and isMount)
		or (bp.detectTabardBuffs and isTabard)
		or (bp.detectMinionBuffs and isMinion)
		or (bp.detectOtherBuffs and isOther)
	local excludeTypes = not bp.excludeBuffTypes
		or not (
      ...
      or (bp.excludeStackable and isStackable)
			...
			or (bp.excludeEnrageBuffs and isEnrage)
			or (bp.excludeMagicBuffs and isMagic)
			or (bp.excludeEffectBuffs and isEffect)
			or (bp.excludeAlertBuffs and isAlert)
			or (bp.excludeWeaponBuffs and isWeapon)
			or (bp.excludeTracking and isTracking)
			or (bp.excludeResources and isResource)
			or (bp.excludeMountBuffs and isMount)
			or (bp.excludeTabardBuffs and isTabard)
			or (bp.excludeMinionBuffs and isMinion)
			or (bp.excludeOtherBuffs and isOther)
		)
	if
		(
			(
				checkAll
				and not (bp.noPlayerBuffs and (id == UnitGUID("player")))
				and not (bp.noPetBuffs and (id == UnitGUID("pet")))
				and not (bp.noTargetBuffs and (id == UnitGUID("target")))
				and not (bp.noFocusBuffs and (id == UnitGUID("focus")))
			)
			or (
				not checkAll
				and not (bp.noPlayerBuffs and UnitIsUnit(unit, "player"))
				and not (bp.noPetBuffs and UnitIsUnit(unit, "pet"))
				and not (bp.noTargetBuffs and UnitIsUnit(unit, "target"))
				and not (bp.noFocusBuffs and UnitIsUnit(unit, "focus"))
				and MOD:CheckCastBy(tc, bp.detectBuffsCastBy)
			)
		)
		and CheckTimeAndDuration(bp, aura[2], aura[5])
		and includeTypes
		and excludeTypes
	then
		...
		if unit == "all" then
			...
			if vbp.noHeaders then
				label = (vbp.noLabels and "" or (label .. (vbp.noTargets and "" or " - "))) .. (vbp.noTargets and "" or gname)
			end
		end
...
local function DetectNewDebuffs(unit, n, aura, isBuff, bp, vbp, bg)
  ...
  local isStackable = aura[3] > 0
    ...
    or (bp.detectMultiStack and isStackable)
		...
		or (bp.detectPoison and isPoison)
		or (bp.detectCurse and isCurse)
		or (bp.detectMagic and isMagic)
		or (bp.detectDisease and isDisease)
		or (bp.detectEffectDebuffs and isEffect)
		or (bp.detectAlertDebuffs and isAlert)
		or (bp.detectOtherDebuffs and isOther)
	local excludeTypes = not bp.excludeDebuffTypes
		or not (
      ...
      or (bp.excludeMultiStack and isStackable)
			...
			or (bp.excludePoison and isPoison)
			or (bp.excludeCurse and isCurse)
			or (bp.excludeMagic and isMagic)
			or (bp.excludeDisease and isDisease)
			or (bp.excludeEffectDebuffs and isEffect)
			or (bp.excludeAlertDebuffs and isAlert)
			or (bp.excludeOtherDebuffs and isOther)
		)
	if
		(
			(
				checkAll
				and not (bp.noPlayerDebuffs and (id == UnitGUID("player")))
				and not (bp.noPetDebuffs and (id == UnitGUID("pet")))
				and not (bp.noTargetDebuffs and (id == UnitGUID("target")))
				and not (bp.noFocusDebuffs and (id == UnitGUID("focus")))
			)
			or (
				not checkAll
				and not (bp.noPlayerDebuffs and UnitIsUnit(unit, "player"))
				and not (bp.noPetDebuffs and UnitIsUnit(unit, "pet"))
				and not (bp.noTargetDebuffs and UnitIsUnit(unit, "target"))
				and not (bp.noFocusDebuffs and UnitIsUnit(unit, "focus"))
				and MOD:CheckCastBy(tc, bp.detectDebuffsCastBy)
			)
		)
		and CheckTimeAndDuration(bp, aura[2], aura[5])
		and includeTypes
...
local function CheckCooldownType(coolDown, bp)
	...
	elseif type == "text" then -- might be a potion or elixir
		...
		elseif name == "Global Cooldown" then
			other = false
			if bp.detectGlobalCooldown then
				return true
			end
		end
	end
...
local function CheckSharedCooldowns(b, bp)
	...
	if MOD.myClass == "WARLOCK" then
		if bp.detectSharedGrimoires then
			...
			if id == 111859 then
				b.barLabel = SHIM:GetSpellInfo(216187)
				return true
			end
		end
...
local function CheckShow(bp)
	...
	if
		...
		or (not MOD.db.profile.hideBlizz and not bp.showBlizz)
		or (MOD.db.profile.hideBlizz and not bp.showNotBlizz)
		or (stat.isResting and not bp.showResting)
		or (stat.isStealthed and not bp.showStealth)
		or (stat.isMounted and not bp.showMounted)
		or (stat.inVehicle and not bp.showVehicle)
		or (stat.targetEnemy and not bp.showEnemy)
		or (stat.targetFriend and not bp.showFriend)
		or (stat.targetNeutral and not bp.showNeutral)
		or (stat.inInstance and not bp.showInstance)
		or (not stat.inInstance and not bp.showNotInstance)
		or (stat.inArena and not bp.showArena)
		or (stat.inBattleground and not bp.showBattleground)
		or (UnitIsUnit("focus", "target") and not bp.showFocusTarget)
		or (bp.showClasses and bp.showClasses[MOD.myClass])
		or (bp.showSpecialization and bp.showSpecialization ~= "" and not MOD.CheckSpec(bp.showSpecialization, bp.specializationList))
		or (bp.checkCondition and IsOn(bp.condition) and not MOD:CheckCondition(bp.condition))
	then
		return false
	end
	return true
					...
					elseif bt == "Cooldown" then
						...
						if not found and bar.enableReady and aname and (bar.readyNotUsable or MOD:CheckSpellStatus(aname, true) or SHIM:IsUsableItem(aname)) then -- see if need cooldown ready bar
							...
							if not bar.startReady or ((GetTime() - bar.startReady) < bar.readyTime) then
								...
								local _, charges = SHIM:GetSpellChargesByID(aname)
								if charges and charges <= 1 then
									charges = nil
								end -- show max charges on ready bar
...
function MOD:UpdateBars()
	...
	if itemID then
		tabardIcon = SHIM:GetItemIconByID(itemID)
	else
		tabardIcon = nil
	end
